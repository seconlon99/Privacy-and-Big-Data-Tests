...
class ExternalBotHandler:
    ...
    def send_reply(
        ...
        """NEW START"""
        message = self.retrieve_message(message["id"])
        """NEW END"""
...
def run_message_handler_for_bot(
    ...
    def handle_message(message: Dict[str, Any], flags: List[str]) -> None:
        ...
        """NEW START"""
        # Create a simplified message dictionary
        simplified_message = restricted_client.store_message(message)

        # Strip at-mention botname from the message
        if is_mentioned:
            # message['content'] will be None when the bot's @-mention is not at the beginning.
            # In that case, the message shall not be handled.
            simplified_message["content"] = extract_query_without_mention(
                message=message, client=restricted_client
            )
            if simplified_message["content"] is None:
                return

        if is_private_message or is_mentioned:
            message_handler.handle_message(message=simplified_message, bot_handler=restricted_client)
        """NEW END"""
